...
func TestSimulatedBackend(t *testing.T) {
	...
	tx := types.NewContractCreation(0, big.NewInt(0), gas, big.NewInt(1), common.FromHex(code), nil, nil, types.QueueOriginL1ToL2)
...
func TestSimulatedBackend_NonceAt(t *testing.T) {
	...
	tx := types.NewTransaction(nonce, testAddr, big.NewInt(1000), params.TxGas, big.NewInt(1), nil, nil, nil, types.QueueOriginL1ToL2, types.SighashEIP155)
...
func TestSimulatedBackend_SendTransaction(t *testing.T) {
	...
	// create a signed transaction to send
	tx := types.NewTransaction(uint64(0), testAddr, big.NewInt(1000), params.TxGas, big.NewInt(1), nil, nil, nil, types.QueueOriginL1ToL2, types.SighashEIP155)
	signedTx, err := types.SignTx(tx, types.HomesteadSigner{}, testKey)
	if err != nil {
		t.Errorf("could not sign tx: %v", err)
	}
...
func TestSimulatedBackend_PendingNonceAt(t *testing.T) {
	...
	tx = types.NewTransaction(uint64(1), testAddr, big.NewInt(1000), params.TxGas, big.NewInt(1), nil, nil, nil, types.QueueOriginL1ToL2, types.SighashEIP155)
	signedTx, err = types.SignTx(tx, types.HomesteadSigner{}, testKey)
	if err != nil {
		t.Errorf("could not sign tx: %v", err)
	}
