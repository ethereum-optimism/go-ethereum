...
func testGetNodeData(t *testing.T, protocol int) {
	...
	generator := func(i int, block *core.BlockGen) {
		...
		case 0:
			...
			tx, _ := types.SignTx(types.NewTransaction(block.TxNonce(testBank), acc1Addr, big.NewInt(10000), params.TxGas, nil, nil, nil, nil, types.QueueOriginSequencer, types.SighashEIP155), signer, testBankKey)
			block.AddTx(tx)
		case 1:
			// In block 2, the test bank sends some more ether to account #1.
			// acc1Addr passes it on to account #2.
			tx1, _ := types.SignTx(types.NewTransaction(block.TxNonce(testBank), acc1Addr, big.NewInt(1000), params.TxGas, nil, nil, nil, nil, types.QueueOriginSequencer, types.SighashEIP155), signer, testBankKey)
			tx2, _ := types.SignTx(types.NewTransaction(block.TxNonce(acc1Addr), acc2Addr, big.NewInt(1000), params.TxGas, nil, nil, nil, nil, types.QueueOriginSequencer, types.SighashEIP155), signer, acc1Key)
			block.AddTx(tx1)
			block.AddTx(tx2)
		case 2:
...
func testCheckpointChallenge(t *testing.T, syncmode downloader.SyncMode, checkpoint bool, timeout bool, empty bool, match bool, drop bool) {
	...
	pm, err := NewProtocolManager(config, cht, syncmode, DefaultConfig.NetworkId, new(event.TypeMux), new(testTxPool), ethash.NewFaker(), blockchain, db, 1, nil, rollupTransitionBatchBuilder)
	if err != nil {
...
func testBroadcastBlock(t *testing.T, totalPeers, broadcastExpected int) {
	...
	rollupTransitionBatchBuilder := rollup.NewDummyBatchBuilder()
	pm, err := NewProtocolManager(config, nil, downloader.FullSync, DefaultConfig.NetworkId, evmux, new(testTxPool), pow, blockchain, db, 1, nil, rollupTransitionBatchBuilder)
	if err != nil {
		t.Fatalf("failed to start test protocol manager: %v", err)
	}
	pm.Start(1000)
...
func TestBroadcastMalformedBlock(t *testing.T) {
	...
	rollupTransitionBatchBuilder := rollup.NewDummyBatchBuilder()
	pm, err := NewProtocolManager(config, nil, downloader.FullSync, DefaultConfig.NetworkId, new(event.TypeMux), new(testTxPool), engine, blockchain, db, 1, nil, rollupTransitionBatchBuilder)
	if err != nil {
		t.Fatalf("failed to start test protocol manager: %v", err)
	}
	pm.Start(2)
