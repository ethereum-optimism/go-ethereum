...
func genValueTx(nbytes int) func(int, *BlockGen) {
	return func(i int, gen *BlockGen) {
		...
		tx, _ := types.SignTx(types.NewTransaction(gen.TxNonce(benchRootAddr), toaddr, big.NewInt(1), gas, nil, data, nil, nil, nil), types.HomesteadSigner{}, benchRootKey)
		gen.AddTx(tx)
	}
}
...
func genTxRing(naccounts int) func(int, *BlockGen) {
	...
	return func(i int, gen *BlockGen) {
		...
		for {
			...
			to := (from + 1) % naccounts
			tx := types.NewTransaction(gen.TxNonce(ringAddrs[from]), ringAddrs[to], benchRootFunds, params.TxGas, nil, nil, nil, nil, nil)
