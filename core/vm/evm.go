...
func (evm *EVM) Call(caller ContractRef, addr common.Address, input []byte, gas uint64, value *big.Int) (ret []byte, leftOverGas uint64, err error) {
	...
	if UsingOVM {
		...
		if caller.Address() == OvmExecutionManager.Address &&
			...
			log.Debug("Set original target address", "target", evm.Context.OriginalTargetAddress.Hex())
		}
	}
	...
	if UsingOVM {
		...
		if evm.depth == 0 {
			...
			log.Debug("Real result is", "ret", hexutil.Encode(evm.Context.OriginalTargetResult))
			...
			if evm.Context.OriginalTargetReached {
				log.Debug("Reached the end of an OVM execution", "Return Data", hexutil.Encode(ret), "Original Target", evm.Context.OriginalTargetAddress.Hex(), "Error", err)
			} else {
