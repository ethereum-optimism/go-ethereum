...
func createBlocks(number int, startIndex int, withTx bool) types.Blocks {
	...
	for i := 0; i < number; i++ {
		...
		if withTx {
			tx, _ := types.SignTx(types.NewTransaction(uint64(i), testUserAddress, big.NewInt(1), params.TxGas, big.NewInt(0), nil, &testUserAddress, &testRollupTxId, nil), types.HomesteadSigner{}, testBankKey)
			txs = append(txs, tx)
		}
